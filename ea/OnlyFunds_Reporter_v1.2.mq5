...
#property description "Sends trading data + EA-computed today_pl to OnlyFunds Dashboard (report-only)"
...
void SendData()
{
   ...
   if(res == 200)
   {
      if(!g_initDone)
      {
         Print("✓ OnlyFunds: Connected! v1.2 | broker offset ", (int)brokerOffsetSec, "s | today P/L: ", DoubleToString(todayPl, 2), " (", closedToday, " deals)");
         g_initDone = true;
      }
   }
   else if(res == 404)
