...
int OnInit()
{
   ...
   EventSetTimer(1);
   Print("DOI DASH Reporter v1.1 started | Account: ", AccountInfoInteger(ACCOUNT_LOGIN));
...
void SendData()
{
   ...
   if(res == 200)
   {
      if(!g_initDone)
      {
         Print("✓ DOI DASH: Connected! Account online (v1.1, broker offset ", (int)brokerOffsetSec, "s)");
         g_initDone = true;
      }
   }
   else if(res == 404)
